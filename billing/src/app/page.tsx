...
export default function PricingPage() {
  const { lang, currency, setCurrency, rates } = useBilling();

  const getPlanPrice = (baseUSD: number) => {
    let price = baseUSD;
    let symbol = 'USD';
    if (currency === 'EGP') {
      price = baseUSD * rates.EGP;
      symbol = lang === 'ar' ? 'ج.م' : 'EGP';
    } else if (currency === 'SAR') {
      price = baseUSD * rates.SAR;
      symbol = lang === 'ar' ? 'ر.س' : 'SAR';
    } else {
      symbol = lang === 'ar' ? '$' : 'USD';
    }
    const formattedPrice = price.toLocaleString(lang === 'ar' ? 'ar-EG' : 'en-US', {
      minimumFractionDigits: 0,
      maximumFractionDigits: 2
    });
    return { priceText: formattedPrice, periodText: symbol + ' / ' + (lang === 'ar' ? 'شهرياً' : 'Month') };
  };
  ...
  const getTrialPeriod = () => {
    ...
    return symbol + ' / ' + (lang === 'ar' ? '7 أيام' : '7 Days');
  };

  const basicInfo = getPlanPrice(60);
  const proInfo = getPlanPrice(100);
  ...
  return (
    <div className="pricing-section">
      <div className="hero">
        ...
        <p style={{ fontFamily: 'var(--font-ar)', maxWidth: '650px', margin: '0 auto', marginBottom: '24px', color: 'var(--text-muted)' }}>
          {t.heroDesc}
        </p>
      </div>
      ...
      <div className="pricing-grid">
        ...
        <div className="pricing-card">
          <div className="pricing-header">
            ...
            <div className="price">0 <span style={{ fontSize: '1rem', color: 'var(--text-muted)', fontWeight: 500 }}>{getTrialPeriod()}</span></div>
        ...
        <div className="pricing-card">
          <div className="pricing-header">
            ...
            <div className="price">{basicInfo.priceText} <span style={{ fontSize: '1rem', color: 'var(--text-muted)', fontWeight: 500 }}>{basicInfo.periodText}</span></div>
